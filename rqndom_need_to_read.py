...
class UltrasoundRenderer:
    ...
    def trace_ray(
        ...
        print(W,D,H)
        grid[..., 0] = 2 * (points[..., 0] / (W - 1)) - 1  # x → width
        grid[..., 1] = 2 * (points[..., 1] / (H - 1)) - 1  # y → height
        grid[..., 2] = 2 * (points[..., 2] / (D - 1)) - 1  # z → depth
        grid = grid.view(1, -1, num_samples, 1, 3)
        print(grid, points)
        if False:  # DEBUG
            print("[INFO] Grid shape:", grid.shape, "Z shape:", Z.shape)
        # Sampling
        ray_values = F.grid_sample(
            Z, grid, mode='bilinear', padding_mode='border'
        ).squeeze()
        return ray_values
    ...
    def trace_rays(self, volume, sources, directions, num_samples):
        ...
        all_profiles = []
        for src in sources:
            profile = self.trace_ray(volume, src, directions, num_samples, num_rays=2)
    ...
    def plot_beam_frame(
        ...
        **kwargs  # for future extensibility, e.g. MRI=False
        ...
        # plt.imshow(processed_output, cmap='gray', aspect='auto')
        # Attenuation model
    ...
    max_sigma=max_sigma)
        ...
        x_coords, z_coords = infer_fan_plane(
            start=source,
            directions=directions,
            n_samples=n_samples,
            start_depth=start
        ...
        # print(f"[INFO] x_coords: {x_coords.shape}, z_coords: {z_coords.shape}, intensities: {intensities.shape}")
        if not plot:
        ...
        # Initial plot
        img = differentiable_splat(x_coords, z_coords, intensities, H=256, W=256, sigma=2.0)
        # scatter = ax.scatter(
        #     x_coords, z_coords, c=intensities,
        #     cmap=cmap if cmap is not None else 'gray',
        #     s=2, vmin=threshold_min, vmax=threshold_max * 1.5
        # )
        ax.imshow(img, cmap='gray', aspect='auto', vmin=-0.5, vmax=threshold_max * 1.5)
        ax.set_aspect('equal')
        ...
        # 4. 
        return processed_output_torch, ax
...
def infer_fan_plane(start: torch.Tensor,
                    directions: torch.Tensor,
                    n_samples: int,
                    start_depth: int = 0):
    """
    Projects rays into a plane inferred from direction vectors.
    Returns 2D coordinates (u, v) of sample points along rays.
    """
    ...
    # Get two orthogonal vectors that span the fan plane
    # Pick the first direction as base
    base = directions[0]
    # Find an orthogonal vector (projected out of base)
    diff = directions - (directions @ base)[:, None] * base
    # Remove zero rows (parallel)
    diff = diff[diff.norm(dim=1) > 1e-6]
    
    if diff.shape[0] == 0:
        raise ValueError("All directions are collinear — cannot infer a plane")

    # Use SVD/PCA to get basis in the fan plane
    U, S, Vh = torch.linalg.svd(diff)
    u_axis = Vh[0]
    v_axis = Vh[1]

    # Sample points
    steps = torch.arange(n_samples, dtype=torch.float32, device=device).view(1, n_samples, 1)
    directions = directions.view(n_rays, 1, 3)
    points = start.view(1, 1, 3) + steps * directions
    points = points[:, start_depth:, :]

    # Project onto (u_axis, v_axis)
    u_coords = torch.tensordot(points, u_axis, dims=([2], [0]))
    v_coords = torch.tensordot(points, v_axis, dims=([2], [0]))

    return u_coords.flatten(), v_coords.flatten()
